class Dice
    ...
    def score_on_roll(dice)
        ...
        # initialize counter variable to be equal to 0, we will be using counter variable to keep a track of the index
        counter = 0
        scoring_dice = 0                                # to keep a track of scoring dices, If all of the thrown dice are scoring, then the player may roll all 5 dice in the next roll.
        for i in 0...dice.size
            ...
            if i == counter                             # initially i and counter both are 0
                if dice[counter] == 1 && dice[counter+1] == 1 && dice[counter+2] == 1
                    ...
                    element_to_be_deleted_from_index << counter
                    element_to_be_deleted_from_index << counter+1
                    element_to_be_deleted_from_index << counter+2
                    counter += 3                        # also counter is incremented by 3
                    scoring_dice += 3                   # and scoring_dice gets incremented by 3
                    ...
                    scoring_dice += 1                   # and scoring_dice gets incremented by 1
        ...
        element_to_be_deleted_from_index.reverse!
        element_to_be_deleted_from_index.each do |element_at_index|
            dice.delete_at(element_at_index)
        end
        [sum, scoring_dice, dice]                        # return sum and scoring_dice as an array
    end
end