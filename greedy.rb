...
require "./dice-roll"                                                                                                                                               # we will require the class files for our project
require "./dice-score"
...
number_of_players = gets.chomp.to_i                                                                                                                                 # gets number of users playing our game
...
rounds_to_reach_3000 = -1                                                                                                                                           # initialize second last round with -1

dice_roll_object = DiceRoll.new                                                                                                                                     # object dice_roll_object is created for class DiceRoll
dice_score_object = DiceScore.new                                                                                                                                   # object dice_score_object is created for class DiceScore

until is_round_completed                                                                                                                                            # game will continue till the time any player reaches a score of 3000 and set the flag is_round_completed = true
    puts ""
    puts "----------------------------------------------------------------"
    puts "                              Turn #{current_round}"
    puts "----------------------------------------------------------------"
    current_player = 0                                                                                                                                              # begin with first player whose indexing is 0
    number_of_players.times do                                                                                                                                      # iterate as many number of times as the players playing
        ...
        all_5_dice_rolled_in_first_attempt = true                                                                                                                   # all_5_dice_rolled_in_first_attempt will help us to know whether or not all the 5 dices scored point
        while go_for_next_roll == "y"                                                                                                                               # condition to check if player wants to continue further or not
            ...
            if score == 0                                                                                                                                           # if no score is generated, then player loses its turn and also the score generated from that turn also gets lost
                ...
                if dice_count > 0                                                                                                                                   # check if dice_count is > 0, means not all dice have scored points
                    ...
                    go_for_next_roll = (gets.chomp.to_s.downcase)[0]                                                                                                # gets player input if they further want to roll the dice, if "y" follow same procedure again or else go to next player
        ...
        current_player += 1                                                                                                                                         # next player plays the game
    end
